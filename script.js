...
// GLOBAL EVENT LISTENER (Fixes Buttons on Vercel)
...
document.addEventListener("click", function (e) {
  ...
  if (e.target.closest(".btn-add-cart")) {
    ...
    const card = btn.closest(".product-card");
    if (card) {
      // Try to get data from attribute (if added by render) or manually parse
      let productData;
      if (card.dataset.product) {
        productData = JSON.parse(card.dataset.product);
      } else {
        // Fallback if dataset isn't there
        const title = card.querySelector(".product-title").innerText;
        const priceText = card
          .querySelector(".price-current")
          .innerText.replace(/[^0-9.]/g, "");
        const img = card.querySelector("img").src;
        productData = { title, price: priceText, image: img, quantity: 1 };
      }
  ...
  if (e.target.closest(".btn-quick-view")) {
    ...
    if (card && card.dataset.product && window.quickViewModal) {
      window.quickViewModal.showQuickView(JSON.parse(card.dataset.product));
    }
  }

  // 3. CART MODAL ACTIONS (Close, Continue Shopping)
  if (
    e.target.matches(".close") ||
    e.target.id === "continue-shopping" ||
    e.target.id === "cart-modal"
  ...
  // 4. AUTH / LOGIN / SIGNUP BUTTONS
  if (e.target.closest(".auth-action")) {
    e.preventDefault();
    const action = e.target.closest(".auth-action").dataset.action;
    ...
    // If logout
    if (action === "logout" && typeof logoutUser === "function") {
      logoutUser();
    }
  ...
  // 5. OPEN CART (Header Icon)
  if (e.target.closest(".cart-btn") || e.target.closest(".cart-wrapper")) {
